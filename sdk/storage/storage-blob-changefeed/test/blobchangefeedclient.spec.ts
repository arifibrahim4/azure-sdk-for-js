...
describe("Change feed event schema test", async () => {
  ...
  it("Event schema v1 test", async () => {
    ...
    assert.equal(new Date("2022-02-17T12:59:41.4003102Z").valueOf(),
      changeFeedEvent.eventTime.valueOf());
    assert.equal(
      "322343e3-8020-0000-00fe-233467066726",
      changeFeedEvent.id);
    assert.equal(
      "PutBlob",
      changeFeedEvent.data.api);
    assert.equal(
      "f0270546-168e-4398-8fa8-107a1ac214d2",
      changeFeedEvent.data.clientRequestId);
    assert.equal(
      "322343e3-8020-0000-00fe-233467000000",
      changeFeedEvent.data.requestId);
    assert.equal(
      "0x8D9F2155CBF7928",
      changeFeedEvent.data.etag);
    assert.equal(
      "application/octet-stream",
      changeFeedEvent.data.contentType);
    assert.equal(
      128,
      changeFeedEvent.data.contentLength);
    assert.equal(
      "BlockBlob",
      changeFeedEvent.data.blobType);
    assert.equal(
      "https://www.myurl.com",
      changeFeedEvent.data.url);
    assert.equal(
  ...
  it("Event schema v3 test", async () => {
    ...
    assert.equal(
      ...
      changeFeedEvent.eventTime.valueOf());
    assert.equal(
      "eefe8fc8-8020-0000-00fe-23346706daaa",
      changeFeedEvent.id);
    assert.equal(
      "PutBlob",
      changeFeedEvent.data.api);
    assert.equal(
      ...
      changeFeedEvent.data.clientRequestId);
    assert.equal(
      "eefe8fc8-8020-0000-00fe-233467000000",
      changeFeedEvent.data.requestId);
    assert.equal(
      "0x8D9F216266170DC",
      changeFeedEvent.data.etag);
    ...
    assert.equal("2022-02-17T13:08:42.4825913Z", changeFeedEvent.data.previousInfo?.softDeleteSnapshot);
    assert.ok(changeFeedEvent.data.previousInfo?.wasBlobSoftDeleted);
    ...
    assert.equal(
      ...
      changeFeedEvent.data.snapshot);

    assert.equal("ContentLanguage", changeFeedEvent.data.updatedBlobProperties!["ContentLanguage"].propertyName);
    ...
    assert.equal("CacheControl", changeFeedEvent.data.updatedBlobProperties!["CacheControl"].propertyName);
    assert.equal("max-age=100", changeFeedEvent.data.updatedBlobProperties!["CacheControl"].newValue);
    assert.equal("max-age=99", changeFeedEvent.data.updatedBlobProperties!["CacheControl"].oldValue);

    assert.equal("ContentEncoding", changeFeedEvent.data.updatedBlobProperties!["ContentEncoding"].propertyName);
    assert.equal("gzip, identity", changeFeedEvent.data.updatedBlobProperties!["ContentEncoding"].newValue);
    assert.equal("gzip", changeFeedEvent.data.updatedBlobProperties!["ContentEncoding"].oldValue);

    assert.equal("ContentMD5", changeFeedEvent.data.updatedBlobProperties!["ContentMD5"].propertyName);
    assert.equal("Q2h1Y2sgSW51ZwDIAXR5IQ==", changeFeedEvent.data.updatedBlobProperties!["ContentMD5"].newValue);
    assert.equal("Q2h1Y2sgSW=", changeFeedEvent.data.updatedBlobProperties!["ContentMD5"].oldValue);

    assert.equal("ContentDisposition", changeFeedEvent.data.updatedBlobProperties!["ContentDisposition"].propertyName);
    assert.equal("attachment", changeFeedEvent.data.updatedBlobProperties!["ContentDisposition"].newValue);
    assert.equal("", changeFeedEvent.data.updatedBlobProperties!["ContentDisposition"].oldValue);

    assert.equal("ContentType", changeFeedEvent.data.updatedBlobProperties!["ContentType"].propertyName);
    assert.equal("application/json", changeFeedEvent.data.updatedBlobProperties!["ContentType"].newValue);
    assert.equal("application/octet-stream", changeFeedEvent.data.updatedBlobProperties!["ContentType"].oldValue);
  });
  ...
  it("Event schema v4 test", async () => {
    ...
    assert.equal(
      ...
      changeFeedEvent.eventTime.valueOf());
    assert.equal(
      "ca76bce1-8020-0000-00ff-23346706e769",
      changeFeedEvent.id);
    assert.equal(
      "PutBlob",
      changeFeedEvent.data.api);
    assert.equal(
      ...
      changeFeedEvent.data.clientRequestId);
    assert.equal(
      "ca76bce1-8020-0000-00ff-233467000000",
      changeFeedEvent.data.requestId);
    assert.equal(
      "0x8D9F2169F42D701",
      changeFeedEvent.data.etag);
    ...
    assert.equal(
      ...
      changeFeedEvent.data.blobVersion);
    assert.equal(
      "0000000000000001",
      changeFeedEvent.data.containerVersion);
    assert.equal(
      "Archive",
      changeFeedEvent.data.blobAccessTier);
    assert.equal(
      "https://www.myurl.com",
      changeFeedEvent.data.url);
    assert.equal(
    ...
    assert.equal("Hot", changeFeedEvent.data.asyncOperationInfo?.destinationAccessTier);
    assert.ok(changeFeedEvent.data.asyncOperationInfo?.isAsync);
    assert.equal("copyId", changeFeedEvent.data.asyncOperationInfo?.copyId);
  });
  ...
  it("Event schema v5 test", async () => {
    ...
    assert.equal(
        ...
        changeFeedEvent.eventTime.valueOf());
    assert.equal(
        "62616073-8020-0000-00ff-233467060cc0",
        changeFeedEvent.id);
    assert.equal(
        "PutBlob",
        changeFeedEvent.data.api);
    assert.equal(
        ...
        changeFeedEvent.data.clientRequestId);
    assert.equal(
        "62616073-8020-0000-00ff-233467000000",
        changeFeedEvent.data.requestId);
    assert.equal(
        "0x8D9F2171BE32588",
        changeFeedEvent.data.etag);
    ...
    assert.equal("2022-02-17T13:12:11.5726507Z", changeFeedEvent.data.previousInfo?.softDeleteSnapshot);
    assert.ok(changeFeedEvent.data.previousInfo?.wasBlobSoftDeleted);
    assert.equal("2024-02-17T16:11:52.0781797Z", changeFeedEvent.data.previousInfo?.newBlobVersion);
    assert.equal("2022-02-17T16:11:52.0781797Z", changeFeedEvent.data.previousInfo?.oldBlobVersion);
    assert.equal("Hot", changeFeedEvent.data.previousInfo?.previousTier);
